...
Klipper, Moonraker, and ROS simulation assets live under this directory. The ROS
2 workspace is isolated in `ros/`.

All ROS and Gazebo commands are intended to run inside the `ros` Docker Compose
service defined in `ros/docker-compose.yml`. Run host commands from
[`sim/ros/`](/home/evank/code/rphys/pentos/sim/ros), then enter the container
with `docker compose exec ros bash`. The launch file name is `sim.launch.py`.
...
For the Klipper and Moonraker Docker setup, Moonraker commands, and test G-code,
